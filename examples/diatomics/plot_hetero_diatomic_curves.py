...
E_TOO_LOW = -20
for model in ("mace-small", "mace-medium"):
    lzma_path = f"{module_dir}/homo-nuclear-{model}.json.xz"
    with lzma.open(lzma_path, mode="rt") as file:
    ...
    n_too_low = sum(val < E_TOO_LOW for val in min_energies.values())
    print(f"{n_too_low} diatomic curves for {model=} that dip below {E_TOO_LOW=} eV")
