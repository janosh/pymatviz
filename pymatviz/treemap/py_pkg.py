...
def py_pkg_treemap(
    ...
    **kwargs: Any,
    ...
    # Generate hover text for each module
    def _create_hover_text(row: pd.Series) -> str:
        # Use leaf_label (module name) for the bold title
        # repo_path_segment contains the full path including package prefix
        display_path = row["repo_path_segment"]

        parts = [
            f"<b>{row['leaf_label']}</b>",
            f"Path: {display_path}",
            f"Lines: {row['line_count']:,}",
        ]
        # Calculate percentage relative to the raw package total
        if (total := row["line_count"]) > 0 and (
            pkg_total := package_totals.get(row["package_name_raw"], 0)
        ) > 0:
            percent = total / pkg_total
            parts.append(f"{percent:,.1%} of {row['package_name_raw']}")

        # Add analysis results conditionally
        analysis_map = {
            "Classes": "n_classes",
            "Functions": "n_functions",
            "Internal Imports": "n_internal_imports",
            "External Imports": "n_external_imports",
            "Type Imports": "n_type_checking_imports",
        }
        for label, col in analysis_map.items():
            if pd.notna(row[col]):  # Check if value is not None/NaN
                parts.append(f"{label}: {int(row[col])}")  # Ensure integer display

        return "<br>".join(parts)

    df_treemap["hover_text"] = df_treemap.apply(_create_hover_text, axis=1)
    ...
    # Define custom data for hover/linking
    # Order: [repo_path_segment, leaf_label, file_url] - Matches test expectations
    custom_data_cols: list[str] = ["repo_path_segment", "leaf_label", "file_url"]
    ...
    fig = px.treemap(
        ...
        values="line_count",
        custom_data=custom_data_cols if processed_base_url else None,
        **treemap_defaults | kwargs,
    )

    # Configure hover display
    # Use the generated hover_text column, disable default hovertemplate
    fig.update_traces(hoverinfo="text", hovertext=df_treemap["hover_text"])
    fig.data[0].hovertemplate = None  # Ensure no conflicting hovertemplate
    ...
    if processed_base_url:  # If link generation is enabled
        ...
        # Assumes file_url (customdata[2]) is always present (even if empty).
        # Broken/empty links are acceptable if URL couldn't be generated.
        link_part = "<a href='%{customdata[2]}' target='_blank'>%{customdata[1]}</a>"
    ...
    # Default behavior when base_url is not provided
        ...
        fig.data[0].texttemplate = "%{label}<br>%{value:,} lines<br>%{percentEntry}"
    elif show_counts is False:
