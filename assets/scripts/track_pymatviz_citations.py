...
def main(update_freq_days: int = 7) -> None:
    ...
    if not os.path.isfile(zotero_file):
        raise FileNotFoundError(
            f"Required Zotero export not found: {zotero_file}. "
            "Export it to this path before running the citation tracker."
        )
    ...
    if not isinstance(zotero_papers, list):
        raise TypeError(f"Expected 'references' to be a list in: {zotero_file}")
