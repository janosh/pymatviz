...
def test_annotate_invalid_fig() -> None:
    with pytest.raises(TypeError, match="Expected plotly Figure"):
        pmv.utils.annotate("test", fig="invalid")  # type: ignore[invalid-argument-type]
...
def test_get_fig_xy_range(plotly_scatter: go.Figure) -> None:
    ...
    # error message
    with pytest.raises(TypeError, match="Expected plotly Figure"):
        pmv.utils.get_fig_xy_range(fig="invalid")  # type: ignore[invalid-argument-type]
...
def test_get_font_color_invalid_input() -> None:
    ...
    with pytest.raises(
        ...
        pmv.utils.get_font_color(fig)  # type: ignore[invalid-argument-type]
