...
def test_structure_2d_axis(axis: str | bool) -> None:
    ax = structure_2d(disordered_struct, axis=axis)
    assert isinstance(ax, plt.Axes)
...
def test_structure_2d_site_labels(
    ...
    ax = structure_2d(disordered_struct, site_labels=site_labels)
    assert isinstance(ax, plt.Axes)
...
def test_structure_2d_warns() -> None:
    ...
    try:
        with pytest.warns(
            ...
            structure_2d(disordered_struct, standardize_struct=standardize_struct)
    finally:
    ...
    with pytest.warns(
        ...
        structure_2d(disordered_struct, subplot_kwargs={"facecolor": "red"})
...
def test_structure_2d_multiple() -> None:
    ...
    fig, axs = structure_2d(struct_dict, n_cols=3)
    ...
    fig, axs = structure_2d(struct_series)
    assert axs.shape == (4,)  # default n_cols=4
    assert [ax.get_title() for ax in axs.flat] == sub_titles
    # Test subplot_kwargs
    fig, axs = structure_2d(struct_series, subplot_kwargs={"squeeze": False})
    assert axs.shape == (1, 4)  # default n_cols=4

    # Test list[Structure]
    fig, axs = structure_2d(list(struct_dict.values()), n_cols=2)
    ...
    fig, axs = structure_2d(struct_series, subplot_title=subplot_title)
    sub_titles = [
...
def test_structure_2d_color_warning() -> None:
    ...
    for colors in ElemColorScheme:
        with pytest.warns(
            ...
            structure_2d(struct, elem_colors=colors)
    ...
    with pytest.warns(UserWarning, match="'O' not in colors, using gray"):
        structure_2d(struct, elem_colors=custom_colors)
...
def test_structure_2d_color_schemes() -> None:
    ...
    ax_jmol: plt.Axes = structure_2d(struct, elem_colors=ElemColorScheme.jmol)
    jmol_colors = {
    ...
    ax_vesta: plt.Axes = structure_2d(struct, elem_colors=ElemColorScheme.vesta)
    vesta_colors = {
        patch.elem_symbol: patch.get_facecolor()
